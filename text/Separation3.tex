...
Kot osnovo smo za poizvedbe najbližjega soseda uporabili funkcijo $search$ v razredu $Kd{\_}tree$, ki kot argument sprejme $OutputIterator$, kamor se shranjujejo objekti, ki jih poizvedba vrne, in $FuzzyQueryItem$, ki je v dvodimenzionalnem primeru lahko krog ali pravokotnik in določa območje iskanja. Kot argument lahko sprejme vrednost $\epsilon$, ki določa stopnjo mehkosti (ang. fuzzyness) in se jo uporablja pri aproksimacijskih poizvedbah, ki pa jih v našem algoritmu nismo potrebovali. Tip, ki smo ga uporabili, je $Fuzzy\U circle$ s središčem, ki ga določa točka poizvedbe in polmerom $1$. Časovna kompleksnost funkcije $search$ je enaka $\OO(\log n + k)$, kjer je $k$ število vrnjenih točk znotraj območja iskanja. Ker naš algoritem zahteva čas $\OO(\log n)$, smo razredu $Kd{\_}tree$ dodali podobno funkcijo, ki vrne par $(boolean, Point\U 2)$. Ker nas v notranjih vozliščih drevesa najbližjega soseda ne zanimajo konkretne točke, ampak samo informacija, če se vsaj ena točka nahaja v območju iskanja, funkcija iskanje zaključi v tistem trenutku, ko ugotovi, da je ta pogoj izpolnjen ter vrne par $true$ in prvo točko v poddrevesu notranjega vozlišča.

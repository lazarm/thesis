...
%naslovnica
 \thispagestyle{empty}%
    {\noindent \large\sc Univerza v Ljubljani\par%
      \noindent Fakulteta za matematiko in fiziko\\
      Fakulteta za računalništvo in informatiko}%
     {\vskip 1em}
     {\noindent \large Računalništvo in matematika – 2. stopnja}
    \vskip 10em%
    \begin{center}
    {\autfont Lazar Milinković\par}%
    {\titfont Implementacija algoritmov za probleme najkrajših poti v presečnih grafih enotskih krogov\par}%
    {\vskip 2em {\autfont Magistrsko delo}\par}%
    {\autfont Mentor: prof. dr. Sergio Cabello \par}%
\end{center}
\vfill\null%
...
\begin{codebox}
    \Procname{$\proc{BFS}(G,r)$}
    \li \For $u\in V(G) - \{r\}$ \Do
    	\li $visited[u] \gets$ false
    	\li $dist[u] \gets \infty$
...
Vzemimo katerokoli točko $p_j$ v $\pi$, ki je potem vsebovana v $D_{wp}$. Ker $|wp_j| \leq |wp| \leq 1$, velja $d(r, p_j) \leq d(r, w) + 1 = i$. Ker $|p_jp| \leq |wp| \leq 1$, velja $d(r, p_j) \geq d(r, p) - 1 = i - 1$. Ampak izbira $w$ kot točke iz $W_{i-1}$ najbližje $p$ pomeni, da $d(r, p_j) \neq i - 1$, ker $|p_jp| < |wp|$. Zato velja $d(r, p_j) = i$. Iz tega sklenemo, da za vsa notranja vozlišča $p_j$ v $\pi$ velja $d(r, p_j) = i$. 
\end{proof}

Celotni algoritem, ki ga poimenujmo \textit{UnweightedShortestPath}, je prikazan na sliki~\ref{fig:SSSP}.
...
\begin{codebox}
    ...
    \li \While $W_{i-1}\neq\emptyset$ \Do
    	...
    	\li $Q \gets W_{i-1}$ // generator kandidatnih točk
        \li $W_{i}\gets\emptyset$
    	\li \While $Q\neq\emptyset$\Do
    		\li $q$ poljubna točka v $Q$
            \li odstrani $q$ iz $Q$
            \li \For $qp\in E(DT(P))$ \Do
...
\begin{lema}
\label{lema2}
Na koncu algoritma \textit{UnweightedShortestPath(P, r)} velja:
\begin{equation*}
\forall i \in \NN \cup \{0\}:  W_i = \{p\in P \mid d(r, p) = i\}.
\end{equation*}
Poleg tega za vsako točko $p \in P \backslash \{r\}$ velja $dist[p] = d(r,p)$ in, če velja $d(r,p) < \infty$, potem obstaja najkrajša pot v $G$ od $r$ do $p$, v kateri je zadnja povezava $\pi [p]p$.
\end{lema}
...
\begin{align*}
	...
	N[p]+N[q]+\CR(\sigma,pq) = \CR (\sigma, \operatorname{cikel}(T_r,pq))\pmod 2\\
	\forall pq\in E(T_r):~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\\
	N[p]+N[q]+\CR(\sigma,pq) = 0~~~~~~~~~~~~~~~~~~~~~~~ \pmod 2,
\end{align*}
...
\begin{codebox}
    ...
    \li \For $r\in P$ \Do
    	\li $(\dist[~],\pi[~])\gets$ drevo najkrajših poti s korenom $r$ v $G$
		...
		\\ \> // in zgradi $L_i^0,L_i^1,D_i^0,D_i^1$
    	\li $N[r]=0$
...
V nadaljevanju sta opisani implementaciji algoritma za drevo najkrajših poti v grafu enotskih krogov iz~\cite{CJ15}, ki je bil opisan v poglavju~\ref{sssp-tree}, in algoritma za minimalno ločitev z enotskimi krogi, opisanega v poglavju~\ref{compact}. Gre za razširjen opis implementacije iz~\cite{CM2}. Za drevo najkrajših poti, zgrajeno z algoritmom iz~\cite{CJ15}, bomo v naadaljevanju uporabljali kratico SSSP.
...
Najprej se inicializira objekt SSSPTree, ki v konstruktorju zgradi DT. Za vsako točko $r\in P$ se nato zgradi drevo s korenom $r$. Iz drevesa dobimo ven štiri sezname $L0, L1, R0, R1$, kjer so točke organizirane po njihovih razdaljah do korena $r$. V \textit{while} zanki nato preverimo kandidate v istem vrstnem redu, kot so nanizani v sliki~\ref{fig:FULL}. Za kandidate na isti strani daljice $\sigma$ se pokliče metoda \textit{nnSeparation}, za ostale pa \textit{rangeSeparation}.

V metodi \textit{nnSeparation} za vse pare množic točk $(S_1, S_2)$, ki tvorijo cikel dolžine $2i$ in se nahajajo na isti strani daljice $\sigma$, zgradimo kd drevo nad $S_2$, za vsako točko $p\in S_1$ pa v drevesu z območnim iskanjem poiščemo prvo točko $q$, ki je od $p$ oddaljena za največ $1$. To naredimo z našo metodo \textit{search\U exists}. Preprosta optimizacija, ki smo jo dodali v algoritem, je ta, da vnaprej zavržemo tiste pare, pri katerih je ena od množic prazna. S tem bistveno pohitrimo algoritem (glej poglavje~\ref{Rezultati}), saj nima smisla delati poizvedbe nad praznim drevesom ali zgraditi drevesa, nad katerim ne bomo naredili nobene poizvedbe. Shema metode \textit{nnSeparation} je prikazana na sliki~\ref{fig:sep-overall-2}.
...
\begin{codebox}
    ...
    \\ // podobno za $L1, R0, R1$
    \li $pairs \gets [(L_{i-1}^0, L_{i}^1), (L_{i}^0, L_{i-1}^1), (R_{i-1}^0, R_{i}^1),$ 
    \Indentmore \Indentmore 
    			\zi $(R_{i}^0, R_{i-1}^1), (L_{i}^1, L_{i}^0), (R_{i}^1, R_{i}^0)] $ \End \End
    \li \For $(A,B)\in pairs$ \Do

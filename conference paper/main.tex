...
\begin{theorem}[Cabello and Jej\v{c}i\v{c}~\cite{CJ15}]
  ...
  In time $O(n \log n)$ we can compute a shortest path tree from $s$
...
\begin{proof}
	...
	wee for example~\cite[Chapters 5 and 8]{bkos-08}.
...
\item If we want to search the candidates $(p,q)\in L_i^j\times L_{i'}^{1-j}$
	...
	If for some $p$ we get that $|pq_p|\le 11$, then we have
	obtained a closed walk through $r$ of length $i+i'+1$ 
	crossing $st$ an odd number of times.
...
\item If we want to search the candidates $(p,q)\in L_i^j\times R_{i'}^{j}$
	...
	If we get some nonempty set, then we get 
	a closed walk through $r$ of length $i+i'+1$ 
	that crosses $st$ an odd number of times.
...
\item If we want to search the candidates $(p,q)\in L_i^j\times R_{i'}^{1-j}$
	...
	If we get some nonempty set, then we get a cycle
...
\begin{proof}
	...
	and the sets $W_i,L_i^0,L_i^1,R_i^0,L_i^1$ in $O(n\log n)$ time.
...
\begin{varwidth}{\linewidth}
\begin{codebox}
    \Procname{$\proc{SeparationUnitDisksCompactRoot}(P,s,t,r,\best)$}
...
\end{varwidth}
~~~~}\end{center}
\caption{Work for each vertex in the news algorithm for minimum separation with unit disks.}
\label{fig:compact}
\end{figure}
...
Moreover (and this is not described in the algorithm), we can consider first
...
If we use this order, as soon as we find a closed walk of inside 
...
We have implemented the algorithms in Section~\ref{sec:algorithms}
using CGAL (\todo{version}~\cite{reference}
because it provides the more complex procedures we need:
Delaunay triangulations, range trees, and nearest neighboours.
...
CGAL provides procedures to compute the Delaunay triangulation~\cite{}.
For nearest neighbours (in the plane) we used the library by~\cite{}. 
Although in principle one could use also point location on the Voronoi diagram,
our earlier tests suggested using 
We did not test other libraries.

For range trees we extended the structure presented in~\cite{}.

Exact/non-exact arithmetic
...
All the experiments were carried out in a \todo{Computer, environment.}

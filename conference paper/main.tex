...
the minimum number of disks one needs to retain so that any path connecting
...
\begin{figure}[htb]
	\centering
	\includegraphics[page=4,width=.3\textwidth]{unitdisks}
	\hfill
	\includegraphics[page=2,width=.3\textwidth]{unitdisks}
	\hfill
	\includegraphics[page=3,width=.3\textwidth]{unitdisks}
...
that separates $s$ and $t$. See the right of Figure~\ref{fig:example1} for an example.
...
We have implemented the algorithms described using CGAL (\todo{version}.
